...
describe 'the person view', type: :feature do
	...
	describe 'email addresses', type: :feature do
		before(:each) do
			person.email_addresses.create(address: 'oswaldo@bartonboehm.biz')
			person.email_addresses.create(address: 'arvel.satterfield@dooley.com')
			visit person_path(person)
		end
		...
		it 'adds an email address' do
			...
			page.fill_in('Address', with: 'cathy@considineosinski.io')
			page.click_button('Create Email address')
			expect(current_path).to eq(person_path(person))
			expect(page).to have_content('cathy@considineosinski.io')
		end
		...
		it 'edits an email address' do
			...
			first(:link, 'edit').click
			page.fill_in('Address', with: 'xander@hyatt.name')
			page.click_button('Update Email address')
			expect(current_path).to eq(person_path(person))
			expect(page).to have_content('xander@hyatt.name')
			expect(page).to_not have_content(old_email)
		end

		it 'has a delete email address link' do
			person.email_addresses.each do |email|
